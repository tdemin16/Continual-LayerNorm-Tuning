...
def get_args_parser(subparsers: argparse.ArgumentParser):
    ...
    subparsers.add_argument('--method', type=str, 
                            choices=['two_stages', 'single_stage'], 
                            default='two_stages',
                            help='two_stages: forward each image twice, \
                                  single_stage: forward each image once.'
    )
    subparsers.add_argument('--full_match', action='store_true')
    subparsers.add_argument('--unbiased', action='store_true',
                            help='Corresponds to classifier rectification in the paper.')
    subparsers.add_argument('--epochs_unbiased', type=int, default=5)
    subparsers.add_argument('--lr_unbiased', type=float, default=0.001)
    subparsers.add_argument('--detach_selection', action='store_true')
    subparsers.add_argument('--use_att', action='store_true',
                            help='Use alpha parameters')
    subparsers.add_argument('--train_layers', type=str, nargs='*', default=[f'{i}' for i in range(13)], 
                            help='Layers subject to LN tuning')
    subparsers.add_argument('--split_train', action='store_true', help='Split the training of key and model. \
                                                                        Useful when you need the key to be trained \
                                                                        before optimizing the weights.')
    subparsers.add_argument('--new_key', action='store_true', help='Init a new key for every task instead of copying the prev one')
    subparsers.add_argument('--key_init', type=str, default='orthogonal', choices=['uniform', 'orthogonal'], 
                            help='Init key with uniform (-1, 1) or orthognal.')
    subparsers.add_argument('--save_cm', action='store_true', help='Store Confusion matrix in task selection')
    subparsers.add_argument('--wandb', action='store_true', help='Enables Weights and Biases')
    subparsers.set_defaults(split_train=True)
