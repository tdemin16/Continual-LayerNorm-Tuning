...
def is_trainable(args, name: str) -> bool:
    if name.startswith('head'):
        return True
    if name.startswith('fake_head'):
        return True
    if name.endswith('key'):
        return True
    if name.endswith(('w', 'b', 'att')):
        # last ln layer
        if not 'block' in name and '12' in args.train_layers:
            return True
        # check if block number is in trainable layers
        if 'block' in name and name.split('.')[1] in args.train_layers:
            return True
    return False


def get_optimizer(args, model):
    # not required, used in old version of the code
    return create_optimizer(args, model.parameters())
    

def mask_logits(logits: torch.Tensor, mask: torch.Tensor, nb_classes: int, task_id: int):
