...
class VisionTransformer(nn.Module):
    ...
    def forward_features_ln(self, x, original_feats, task=-1, task_eval=None, task_selection=None):
        metrics = {}
        if original_feats is not None:
            tasks, metrics = self.wsm(feats=original_feats, task=task, task_eval=task_eval)
        elif task_selection is not None:
            tasks = task_selection
        else:
            # case split_train
            tasks = torch.full((x.size(0), ), task)
        ...
        for block in self.blocks:
            x, _ = block(x, tasks)
        x, _ = self.mln(x, tasks)
        return x, metrics
    
    def forward_features_single_stage(self, x, task, task_eval):
        assert self.block_fn == BlockLN
        x = self.patch_embed(x)
        x = self._pos_embed(x)

        acc_sim_layer = []
        metrics = {'sim': 0, 'acc_sim': 0}

        # case split_train
        tasks = torch.full((x.size(0), ), task)

        for block in self.blocks:
            x, m = block(x, tasks, task_eval)
            
            metrics = {k: metrics[k] + m[k] for k in metrics.keys()}
            if task_eval is not None:
                acc_sim_layer.append(m['acc_sim'].item())
        
        x, m = self.mln(x, tasks, task_eval)
        
        metrics = {k: metrics[k] + m[k] for k in metrics.keys()}
        if task_eval is not None:
            acc_sim_layer.append(m['acc_sim'].item())
            acc_sim_layer = [round(elem, 2) for elem in acc_sim_layer ]
        metrics['acc_sim_layer'] = acc_sim_layer
        metrics['sim'] /= len(self.blocks) + 1
        metrics['acc_sim'] /= len(self.blocks) + 1
        
        return x, metrics

    def forward_head(self, x, pre_logits: bool=False, fake: bool=False):
        if self.global_pool:
        ...
        if pre_logits:
            return x
        if fake:
            assert hasattr(self, 'fake_head')
            return self.fake_head(x)
        return self.head(x)

    def forward(self, x, original_feats: torch.Tensor=None, task: int=-1, task_eval: int=None, 
                task_selection: torch.Tensor=None, fake: bool=False):
        """
        original_feats: feats to use for weight selection. None when WSM not involved in training
        task: current task during training. -1 when evaluating
        task_eval: current task during evaluation (just for metrics). None when training.
        task_selection: used in case of 'last' weight selection (only eval)
        fake: use fake classifer (used to train weights but not used for evaluating). 
              To be used with unbiased arg
        """
        metrics = {}
        if not hasattr(self, 'method'):
            feats = self.forward_features(x)
        elif self.method == 'two_stages':
            feats, metrics = self.forward_features_ln(x, original_feats, task, task_eval, task_selection)
        elif self.method == 'single_stage':
            feats, metrics = self.forward_features_single_stage(x, task, task_eval)
        else:
            raise NotImplementedError
        
        x = self.forward_head(feats, fake=fake)
        return x, feats, metrics
    
    def init(self, args):
        self.method = args.method
        self.init_MLN(args)

    def init_MLN(self, args):
        assert self.block_fn == BlockLN
        assert args.method in ['two_stages', 'single_stage']

        if args.method == 'two_stages':
            self.wsm = WeightSelectionMechanism(args, embed_dim=self.norm.weight.size(0))
        for block in self.blocks:
            block.init_params(args, self.norm_layer)
        self.mln = MultiTaskLN(args, norm_layer=self.norm_layer, w=self.norm.weight, b=self.norm.bias)

        if args.unbiased:
            self.fake_head = deepcopy(self.head)

    def next_task(self, task: int=None, prev_task: int=None):
        if prev_task is None:
            prev_task = task - 1
        assert prev_task >= 0 and prev_task < task

        if self.block_fn == BlockLN:
            for block in self.blocks:
                block.next_task(task)
            self.mln.next_task(task)
            if hasattr(self, 'wsm'):
                self.wsm.next_task(task)
    
    def get_last_similarity(self):
        return self.wsm.get_last_similarity()

    def set_custom_match(self, mode: bool=False):
        for block in self.blocks:
            block.set_custom_match(mode)
        self.mln.set_custom_match(mode)
        if hasattr(self, 'wsm'):
